...
void Pathfinder::dijkstraForGroups(std::vector<Unit*> units, Tile* target, int groupId) {
	...
	*/
	...
	optimalizoval. Jenûe proË v˘bec zjiöùuji v pr˘bÏhu, zda uû ta polÌËka m·m zanalyzovan·? JednoduööÌ by bylo prostÏ zanalyzovat 
	vöechno, coû trv· max. nÏjak˝ch 150ms. P¯ÌpadnÏ na konci bych jeötÏ mohl zanalyzovat pole s jednotkami, abych zjistil, jestli nÏjak· 
	jednotka t¯eba nestojÌ na poli, ze kterÈho do cÌle nevede cesta.
	*/

	/* TODO
	Trochu pozmÏnit, jak group pathfinding funguje.
	Moment·lnÏ kdyû m·m 2 jednotka d·l od sebe a oznaËÌm cÌl mezi nimi (O - - - - X - - - O), tak jedotka vlevo p˘jde 4 polÌËka
	doleva, a aû potom zjistÌ, ûe m· vlastnÏ jÌt doprava. Proto bych mohl udÏlat check, ûe pokud by prvnÌ krok jednotky (kter˝
	by odpovÌdal kroku leadera) öel v˝raznÏ jinam neû ¯Ìkaj· vektory, pak by ignoroval leadera a ¯Ìdil se jenom vektory.
